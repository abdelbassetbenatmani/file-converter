...
import { CheckCheck, CloudDownload, Download, Loader, LucideFileSymlink, ShieldX, X } from "lucide-react";
...
import compressFileName from "@/lib/compress-file";
      ...
      <div className="space-y-6 z-40">
        {actions.map((action: Action, i: any) => (
          <div
            ...
            {action.is_converted ? (
              <Button className="ml-2" variant="outline" size="sm" onClick={() => download(action)}>
                Download
              </Button>
            ) : (
              <span
  ...
  return (
    <ReactDropzone
      ...
      {({ getRootProps, getInputProps }) => (
        <div
          {...getRootProps()}
          className="w-full z-40 bg-gray-50 h-72 lg:h-80 xl:h-96 rounded-3xl shadow-sm border-2 border-dashed cursor-pointer flex items-center justify-center">
          <input {...getInputProps()} />
